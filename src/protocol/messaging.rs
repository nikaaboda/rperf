...
fn prepare_configuration_tls_upload(
    test_id: &[u8; 16],
    streams: u8,
    bandwidth: u64,
    seconds: f32,
...
fn prepare_configuration_tls_download(
    test_id: &[u8; 16],
    streams: u8,
    length: usize,
    receive_buffer: u32,
...
fn calculate_length_tls(length: usize) -> usize {
...
pub fn prepare_upload_configuration(
    ...
    } else if args.is_present("tls") {
        log::debug!("preparing TLS upload config...");
        if length == 0 {
            length = 32 * 1024;
        }
        if send_buffer != 0 && send_buffer < length {
...
pub fn prepare_download_configuration(
    ...
    if args.is_present("udp") {
        ...
        log::debug!("preparing TLS download config...");
        if length == 0 {
            length = 32 * 1024;
        }
        if receive_buffer != 0 && receive_buffer < length {
        ...
        Ok(prepare_configuration_ktls_download(
            test_id,
            parallel_streams,
            length as usize,
            receive_buffer,
